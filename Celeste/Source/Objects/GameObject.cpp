...
namespace Celeste
{
  ...
  GameObject::~GameObject()
  {
    // Deleting game objects is going to invalidate iterators, so reverse iterate
    for (size_t i = getChildCount(); i > 0; --i)
    {
      delete getChild(i - 1);
    }
  ...
  {
#if _DEBUG
    if (component == nullptr || this != &component->getGameObject())
    {
      ASSERT_FAIL();
      return;
    }
