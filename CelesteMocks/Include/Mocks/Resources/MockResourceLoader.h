...
namespace CelesteMocks
{
  ...
  class MockResourceLoader : public Celeste::Resources::ResourceLoader<T>
  {
    public:
      ...
      bool inMapRelative(const Celeste::Path& relativePath) { return getMap().end() != getMap().find(internString(Celeste::Path(Celeste::Resources::ResourceLoader<T>::getResourceDirectoryPath(), relativePath).as_string())); }

      bool inMapFull(const Celeste::Path& fullPath) { return getMap().end() != getMap().find(internString(fullPath.as_string())); }
  };
}